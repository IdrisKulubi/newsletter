...
import { redisConnection, EmailJobData, AnalyticsJobData, AIJobData } from './index';
...
export const emailWorker = new Worker<EmailJobData>(
  ...
  async (job: Job<EmailJobData>) => {
    const { campaignId, tenantId, recipients, batchSize } = job.data;
    ...
    try {
      ...
      // Simulate email processing (in real implementation, this would use Resend)
      let processed = 0;
      ...
      for (const recipient of recipients) {
        ...
        processed++;
        const progress = Math.round((processed / total) * 90) + 10; // 10-100%
        await job.updateProgress(progress);
        
        console.log(`Sent email to ${recipient.email} (${processed}/${total})`);
      }
      
      console.log(`Email job ${job.id} completed. Processed ${processed} emails.`);
    ...
    try {
      ...
      switch (eventType) {
        case 'campaign-complete':
          ...
          console.log(`Processing campaign completion analytics for ${campaignId}`);
          ...
          return {
            ...
            metrics: {
              ...
              openRate: data?.totalSent ? (data.opened / data.totalSent) * 100 : 0,
            },
